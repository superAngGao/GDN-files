# Technical Report: Agentic TileLang Kernel Tuning For Gated DeltaNet Prefill

This report is the audit trail behind the publication-oriented case study
`tutorial_v4.md`. It prioritizes evidence separation, attribution boundaries,
rejected candidates, ABI caveats, and reproduction metadata over narrative
flow.

Use the two documents together:

- `tutorial_v4.md` is the official case-study entry point: problem, mechanism,
  result, and reusable engineering lessons.
- This report is the companion evidence report: controlled ladders,
  experiment-adapter rows, same-input ablations, failed rows, lowering
...
Report note: detailed raw artifacts live in `tutorial_v4_si.md` and the
`evidence/` tree. The diagrams below are lightweight Mermaid sketches retained
from the working report.
...
Detailed evidence lives in `tutorial_v4_si.md`, including:
